...
// Exercises: Level 2
let countryName = countries.map((a) => a.name);
console.log(countryName);

// let categorizeCountry = countries.filter((av) => {
//   if (av.name.includes("land")) {
//     return av.name;
//   }
// });
...
function categorizeCountries(arr) {
  return arr.filter((con) => con.includes("land"));
}
console.log(categorizeCountries(countryName));
// 4. Create a function which return an array of objects, which is the letter and the number of times the letter use to start with a name of a country

let firstLetter = countryName.map((x) => x.charAt(0));
let newTally = firstLetter.reduce((tally, val) => {
  if (tally[val]) {
    tally[val]++;
  } else {
    tally[val] = 1;
...
function getFirstTenCountries(arr, count) {
  let firstTen = arr.sort((a, b) => (a > b ? 1 : -1)).reverse;
  return firstTen.slice(0, count);
}
// console.log(getFirstTenCountries(countryName, 10))

//6
let lastTen = [];
const newI = countryName.length - 10;
console.log(newI);
function getLastTenCountries(c) {
  for (let i = newI; i < countryName.length; i++) {
    lastTen.push(countryName[i]);
  }
  return lastTen;
}
getLastTenCountries(countryName);
console.log(lastTen);

//7 Ensure you get it done
let onlyCountriesInitial = countries
  .map((a) => a.name.charAt(0))
  .reduce((tally, val) => {
    return { ...tally, [val]: (tally[val] || 0) + 1 };
  }, {});
// console.log(onlyCountriesInitial)
...
let byName = countries.sort((x, y) => {
  return x.name > y.name ? 1 : -1;
});
console.log(byName);
let byCapital = countries.sort((x, y) => {
  return x.capital > y.capital ? 1 : -1;
});
console.log(byName);
...
function mostPopulatedCountries(arr, count) {
  let populated = arr.sort((a, b) => b.population - a.population);
  return populated.slice(0, count);
}
...
const mean = sum / agesLength;
console.log(mean);

//Median
let sortedAges = ages.sort((a, b) => a - b);
console.log(
  sortedAges[(sortedAges.length / 2 - 1 + sortedAges.length / 2) / 2]
);
...
dev.push(ages[i]-mean)
}
console.log(dev)

//mode
// let mode = ages.reduce((a, b) => {
//   return { ...a, [b]: (a[b] || 0) + 1 };
// });


// console.log(mode);
// console.log(Object.keys(mode))
// console.log(Object.entries(mode))
// let newMode = Object.entries(mode).map((sort1, sort2)=> {
//   console.log()
//   console.log(sort1)
// })
